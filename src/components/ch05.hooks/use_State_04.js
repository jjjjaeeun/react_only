...
function App() {
    ...
    // 회원 명단(사실 이 데이터는 SpringBoot에서 받아 와야 함)
    ...
    const AddMember = (event) => {
        ...
        // max() 메소드를 이용하여 최대 값을 추출함.
        const newNextId = Math.max(...members.map((bean) => bean.id)) + 1;

        // concat = 새로운 원소를 추가하는 함수
        // 신규 id와 입력한 데이터를 이전 배열 members에 concat() 함수를 이용하여 추가.
